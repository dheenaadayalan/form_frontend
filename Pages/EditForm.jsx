...
function EditForm({ formId }) {
  ...
  const getForm = async (url, method) => {
    ...
    try {
      ...
      if (responseData.success == true) {
        ...
        setFormItem(responseData.form.formData);
      }
      setLoading(false);
    } catch (error) {
      setLoading(true);
  ...
  const submit = () => {
    ...
    const data = { formItem: formItem, title: formTitle, formID: formId };
    console.log(data);
    postData("https://form-backend-tfpv.onrender.com/api/form/update", data, "POST");
  };
